...
function CustomerForm(props){
        ...
        await fetch(link, {
            ...
            if(response.ok){
                ...
                if(props._id){
                    ...
                    props.response(`Customer successfully ${props.method === 'POST' ? 'added' : 'updated'}.`);
                }else{
                    nav("/customer/")
                }
            }else{
                setRep(false);
                props.response(`Unable to ${props.method === 'POST' ? 'add' : 'update'} customer due to an error.`);
            }
    ...
    return (<>
        ...
        <form className='form' onSubmit={handleSubmit}>
            ...
            <input type="text" required className='form-text-input' name="address" placeholder='Street Address' value={data.address || ""} readOnly={props._edit} onChange={e => setData({...data, ["address"] : e.target.value})}/>
            <input type="text" required className='form-text-input' name="city" placeholder='City' value={data.city || ""} readOnly={props._edit} onChange={e => setData({...data, ["city"] : e.target.value})}/>
            <input type="text" required className='form-text-input' name="zip" placeholder='Zip Code' value={data.zip || ""} readOnly={props._edit} onChange={e => setData({...data, ["zip"] : e.target.value})}/>
            ...
            <input type="text" className='form-text-input' name="special" placeholder='Special Instructions' readOnly={props._edit} onChange={e => setData({...data, ["special"] : e.target.value})}/>
